...
function panel(h) {
  ...
  $('panel').innerHTML = `<h2>${esc(name)}</h2><p class="hint">${esc(groupOf[name] || '')} · <a style="color:var(--accent)" href="/3d?regions=${encodeURIComponent(name)}">View in 3D</a> · <a style="color:var(--accent)" target="_blank" rel="noopener" href="${esc(NG.url({ regions: [name], title: name }))}">Neuroglancer ↗</a></p><table class="kv"><tr><td>Input connections</td><td>${fmt(t.input_connections)}</td></tr><tr><td>Output connections</td><td>${fmt(t.output_connections)}</td></tr><tr><td>Neurons with input here</td><td>${fmt(t.neurons_with_input)}</td></tr>`
